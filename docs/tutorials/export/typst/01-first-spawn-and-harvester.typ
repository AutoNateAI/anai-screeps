...
#show: body => tutorial-doc(title: "Tutorial 01: First Spawn and Harvester", source: "docs/tutorials/01-first-spawn-and-harvester.md", body)

= Tutorial 01: First Spawn and Harvester
<tutorial-01-first-spawn-and-harvester>
#emph[Episode 1: Touchdown]

The hatch is open. The room is quiet. Your collaborator is still
narrating the scan results, but the console is yours now --- welcome to
Day 1, no orientation packet included.

Somewhere else on this planet, other Foundry architects are doing
exactly what you\'re about to do: picking a room, placing a spawn, and
watching whether their first worker can complete one single lap without
dying, getting stuck, or just standing there like it\'s waiting on a
code review. Nobody skips this step. Nobody gets to --- not even the
ones who\'ve shipped to production for a decade.

This is the first system you\'ll ship. Keep it small. Keep it visible.
Everything after this builds on whatever you learn watching it run, so
resist the urge to get clever before you\'ve even proven the loop works.
...
This tutorial is for the local AutoNateAI Screeps server at
`localhost:21025`.
...
In this setup the Steam client is mapped to `autonate` by
`SCREEPS_LOCAL_DEFAULT_USER=autonate`, so the client may not ask for the
password.
...
- Being somewhat close to at least one energy source helps early
  movement.
...
#quote(block: true)[
📸 #strong[Screenshot placeholder:] `Spawn1` freshly placed in the
client, before any creep exists --- this is the visual \"checkpoint\" a
new reader can compare their own room against.
]
...
This is the decision your script re-runs every single tick --- the
entire loop, drawn out:

#align(center)[#image("../assets/01-first-spawn-and-harvester/diagram-01.png", alt: "Mermaid diagram 1")]

Every role in every later episode is a variation on this exact shape:
check state, act or move closer, repeat next tick.
...
This is not about writing perfect code yet. It is about seeing the game
loop clearly.
...
- What should the next tutorial add: upgrading, building, or multiple
  roles?

== Next: Episode 2 --- Two Sources, One Colony
<next-episode-2--two-sources-one-colony>
Your collaborator flags it before you do: \"Harvester1 only knows about
`sources[0]`. The second you spawn a friend for it, they\'re going to
fight over the same source and let the other one sit there untouched ---
real \'two founders, one laptop\' behavior.\"

That\'s the problem Episode 2 picks up: teaching the colony to assign
work instead of guessing at it.
