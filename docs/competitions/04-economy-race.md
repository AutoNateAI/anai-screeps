...
The learner races an NPC economy bot to hit early growth milestones, focusing on reliable harvesting, source assignment, spawn uptime, and controller progress.
...
- Recommended episodes: `docs/tutorials/02-target-selection.md` through `docs/tutorials/05-construction-and-roads.md`.
...
Reach the agreed milestone before the benchmark bot or before a fixed time box expires.
...
- Keep spawn idle time low enough that the learner can explain every idle period.
...
- Both sources are used intentionally or the learner can justify not using both.
- Spawn behavior is role-count driven, not hand-spawned.
- The learner compares their progress to the bot or to their previous run.

## Scoring

| Metric | Full Credit | Partial Credit |
| --- | --- | --- |
| Outcome | Reaches milestone and keeps economy running. | Reaches milestone with manual intervention. |
| Code behavior | Role targets adapt to room state. | Role targets exist but are brittle. |
| Debugging process | Learner identifies one wasted tick pattern. | Learner only reports final RCL. |

## Replay

Use a fresh local room or reset the world state, then run the same benchmark again.
